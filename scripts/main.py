...
def food_preferenses(login, product, food_amount):
    ...
    for num_of_food in range(len(products)):
        gramm_in_xo = float(found_in_database("ru_bread_items.json",
                                              products[num_of_food])["1 ХO є в граммах продукта (новий розрах)"])
        eaten += float(food_amounts[num_of_food])/gramm_in_xo
    available = float(search_user(login)['XO']) - eaten
    return[available, eaten]


def diadram_xo(login, product, food_amount):
    """
    The function that builds diagrams
    """
...
diadram_xo("jbrown", "Йогурт, йогурт", "100, 100")